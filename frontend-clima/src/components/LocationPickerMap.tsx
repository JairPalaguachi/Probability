...
export default function LocationPickerMap({ position, onPositionChange }: LocationPickerMapProps) {
  ...
  useEffect(() => {
    if (position && window.L && leafletMapRef.current) {
  ...
  const getCurrentLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => alert('No se pudo obtener tu ubicación: ' + error.message)
      );
    } else {
    ...
    <div className="location-picker-root">
      ...
      <div className="location-picker-map-container">
        ...
        <div className="location-picker-controls">
          <button onClick={getCurrentLocation} className="location-picker-btn" title="Usar mi ubicación actual">
            <Navigation size={20} />
          </button>
          {position && (
            <button onClick={clearSelection} className="location-picker-btn red" title="Limpiar selección">
              <Trash2 size={20} />
            </button>
          )}
        </div>
